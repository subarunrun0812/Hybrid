...
public class Crouch : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (Input.GetKey(key) || Input.GetKey(KeyCode.C) || forceCrouch == true)
        {
            ...
            if (colliderToLower)
            {
                ...
                if (defaultHeadYLocalPosition.HasValue)
                {
                    Debug.Log("defaultHeadYLocalPosition.HasValueの条件が成立した");
        ...
        {
            //ベッドの下に潜り込んでいる時以外は、heightは通常の値にする
            if (getUnderTheBed == false)
            {
                if (IsCrouched)
                {
    ...
    public void GetUnderTheBedTrue()
    {
        getUnderTheBed = true;
    }
    public void GetUnderTheBedFalse()
    {
        getUnderTheBed = false;
    }
